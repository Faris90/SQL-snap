/*
    ...
    © 2015 by Eckart Modrow. All rights reserved.
    emodrow@informatik.uni-goettingen.de
    with some modifications of ojcets.js
...
//------------------------------------------------- eingefügt 15-6-29 ------------------------------------------
//for code-mapping JavaScript
        var code;
        switch (selector) {
            //case 'setColor': {code = 'context.strokeStyle = "<#1>";'; break;}
            case 'doRepeat':
                { //variable names are numbered before execution
                    code = 'for(var _#i=0;_#i<<#1>;_#i++){\n    <#2>\n};';
                    break;
                }
            case 'doUntil':
                {
            ...
            case 'reportRandom':
                {
                    code = 'Math.round(Math.random()*(<#2>-<#1>)+<#1>)';
                    break;
                }
            case 'reportLessThan':
                {
            ...
            case 'doReport':
                {
                    code = 'setReturnvalue(<#1>);';
                    break;
                }
            case 'reportNewList':
                {
                    code = '((String("<#1>").length===0) ? new List() : new List(String("<#1>").split(" ")))';
                    break;
                }
            case 'reportCONS':
                {
                    code = 'new List().cons(<#1>,<#2>)';
                    break;
                }
            case 'reportListItem':
                {
                    code = '(<#2>.length() < 1 ? new String() : (String("<#1>")==="any") ? <#2>.at(Math.floor(Math.random() * (<#2>.length()) + 1)) : \n\
                                                                               (String("<#1>")==="last") ? <#2>.at(<#2>.length()) :\n\
                                                                               <#1> < 1 ? new String() :  <#2>.at(<#1>))';
                    break;
                }
            case 'reportCDR':
                {
            ...
            case 'doDeleteFromList':
                {
                    code = 'if(<#2>.length() > 0)\n   if("<#1>"==="all") <#2> = new List(); \n   else if("<#1>"==="last") <#2>.remove(<#2>.length()); else <#2>.remove(<#1>);';
                    break;
                }
            case 'doInsertInList':
                {
                    code = 'if(<#3>.length() < 1) {<#3> = new List(); <#3>.add(<#1>);}\n   else if("<#2>"==="any") <#3>.add(<#1>,Math.floor(Math.random() * <#3>.length()) + 1);\n      else if("<#2>"==="last") <#3>.add(<#1>);\n         else <#3>.add(<#1>,<#2>);';
                    break;
                }
            case 'doReplaceInList':
                {
                    code = 'if(<#2>.length() > 0) {if("<#1>"==="any") <#2>.put(<#3>,Math.floor(Math.random() * <#2>.length()) + 1);\n      else if("<#1>"==="last") <#2>.put(<#3>,<#2>.length());\n         else <#2>.put(<#3>,<#1>);}';
                    break;
                }
...
//--------------------------------------------------------- eingefuegt 15-6-29 --------------------------------------
    if (cat === 'SQLsnap') {
        ...
        blocks.push(block('reportSQLlike'));
