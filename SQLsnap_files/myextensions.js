...
Process.prototype.updateSQLmessage = function(lastMessage) { //sets the field in connection and updates the dynamic variable SQLmessage, if existing
    ...
    if (connection.theSprite)
       if(connection.theSprite.variables.parentFrame.vars['lastSQLmessage'])
        connection.theSprite.variables.parentFrame.vars['lastSQLmessage'].value = connection.SQLmessage;
};
...
Process.prototype.doConnectToServer = function(myserver, myuser, mypwd) { //connects to a server
    ...
    if (connection.SQLmessage.substring(0, 2) === 'ok')
        connection.ok = true;
...
SpriteMorph.prototype.doCreateDBvars = function() {
    if (connection.theProcess) {
        ...
        if (!(Array(connection.theDBvars).some(function(x) {
            return x === 'lastSQLmessage';
        }))) {
            this.variables.parentFrame.addVar('lastSQLmessage');
            if (ide) ide.flushBlocksCache('variables');
            this.variables.parentFrame.vars['lastSQLmessage'].value = connection.SQLmessage;
       ...
       if (connection.tables !== null) {
            ...
            for (i = 0; i < myTables.length; i++) {
                if (!(connection.theDBvars.some(function(x) {
                    return x === myTables[i];
                }))) {
                    this.variables.parentFrame.addVar(myTables[i]);
                    if (ide) ide.flushBlocksCache('variables');
                    this.variables.parentFrame.vars[myTables[i]].value = myTables[i];
        ...
        if (connection.columns !== null) {
            ...
            for (i = 0; i < myColumns.length; i++) {
                ...
                if (!(connection.theDBvars.some(function(x) {
                    ...
                    this.variables.parentFrame.addVar(myName);
                    if (ide) ide.flushBlocksCache('variables');
                    this.variables.parentFrame.vars[myName].value = myName;
...
Process.prototype.reportSQLlike = function(a, b) {
    return ""+ a + " like '" + b + "'";
};
...
Process.prototype.doWriteTextfile = function(mytext, myfilename) {
    if(mytext.length > 1023){
        mytext = mytext.substring(0,1023);
    }
    var i = myfilename.indexOf('.');
    if (i < 1)
        myfilename = './textfiles/' + myfilename + '.txt';
    else
        myfilename = './textfiles/' + myfilename.substring(0, i) + '.txt';
    if (connection.server === '') {
        this.updateSQLmessage('ERROR: please connect to server first');
    }
    else {
    ...
    function getPoint(x, y, place) {
        ...
        if (newX >= 0 && newX < ext.x && newY >= 0 && newY < ext.y)
            if (place === 'stage')
                ...
                return new List(new Array(pentrailsdata.data[offset], pentrailsdata.data[offset + 1], pentrailsdata.data[offset + 2]));
    }
    ;
