...
//--------------------------------------------------------- eingefuegt 9-12-13 --------------------------------------
        reportGetRGB: {  
            type: 'reporter',
            category: 'sensing',
            spec: 'getRGB %n %n from %lst3',
            defaults: [0,0,'pentrails']
        },   
        doSetRGB: {
            type: 'command',
            category: 'sensing',
            spec: 'setRGB %n %n %n at %n %n of %lst3',
            defaults: [255,100,255,0,0,'pentrails']
        },
        reportRGBtoHSV: {
            type: 'reporter',
            category: 'sensing',
            spec: 'RGB %n %n %n to HSV',
            defaults: [100,0,0]
        },
//------------------------------------------------------------------------------------------------------------------
        reportKeyPressed: {
...
//--------------------------------------------------------- eingefügt 14-1-20 --------------------------------------
        doWriteTextfile: {
            type: 'command',
            category: 'operators',
            spec: 'write %s to file %s',
            defaults: ['this text','thisfilename']
        },
        reportReadTextfile: {
            type: 'reporter',
            category: 'operators',
            spec: 'read from file %s',
            defaults: ['thisfilename']
        },
        doDeleteTextfile: {
            type: 'command',
            category: 'operators',
...
//--------------------------------------------------------- eingefügt 10-12-13 --------------------------------------
//actually not used
        doMakeDynamicVariable: {  
            type: 'command',
            category: 'variables',
            spec: 'make dynVar %s'
        },
        doDeleteDynamicVariable: {
            type: 'command',
            category: 'variables',
            spec: 'delete dynVar %s'
        },
        doSetDynamicVariable: {
            type: 'command',
            category: 'variables',
            spec: 'set dynVar %s to %s'
        },
        reportGetDynamicVariable: {
            type: 'reporter',
            category: 'variables',
            spec: 'get dynVar %s'
        },
...
//--------------------------------------------------------- eingefügt 14-01-01 --------------------------------------
       doConnectToServer: {
            type: 'command',
            category: 'lists',
            spec: 'connect to server %s %s %s',
            //defaults: ['localhost','root','']
            defaults: ['snapextensions.uni-goettingen.de','snapexuser','snap!user']
        },
        reportGetDatabases: {
            type: 'reporter',
            category: 'lists',
        ...
        reportExecSQLcommand: {
            type: 'reporter',
            category: 'lists',
            spec: 'exec SQL-command %s'
        },
        reportSimpleSelect: {
            type: 'reporter',
            category: 'lists',
            spec: 'SELECT %lst1 %mult%s FROM %mult%s WHERE %b'
        },
        reportFullSelect: {
            type: 'reporter',
            category: 'lists',
            spec: 'SELECT %lst1 %mult%s FROM %mult%s WHERE %b GROUP BY %mult%s HAVING %b ORDER BY %mult%s %lst2 LIMIT %n'
        },
//--------------------------------------------------------- eingefügt 14-01-01 --------------------------------------
...
SpriteMorph.prototype.blockTemplates = function (category) {
 ...
 //------------------------------------------------- eingefügt 2014-1-24 ------------------------------------------
        var code;
        switch(selector){
            //case 'setColor': {code = 'context.strokeStyle = "<#1>";'; break;}
            case 'doRepeat': {code = 'for(var _#i=0;_#i<<#1>;_#i++){\n    <#2>\n};'; break;}
            case 'doUntil': {code = 'do{\n    <#2>\n}while(!(<#1>));'; break;}
            case 'doIf': {code = 'if(<#1>){\n    <#2>\n};'; break;}
            case 'doIfElse': {code = 'if(<#1>){\n    <#2>\n}\nelse{\n    <#3>\n};'; break;}
            case 'reportGetRGB': {code = 'getPoint(<#1>,<#2>,"<#3>")'; break;}
            case 'doSetRGB': {code = 'setPoint(<#1>,<#2>,<#3>,<#4>,<#5>,"<#6>");'; break;}
            case 'reportSum': {code = '<#1> + <#2>'; break;}
            case 'reportDifference': {code = '<#1> - <#2>'; break;}
            case 'reportProduct': {code = '<#1> * <#2>'; break;}
            case 'reportQuotient': {code = '<#1> / <#2>'; break;}
            case 'reportModulus': {code = '<#1> % <#2>'; break;}
            case 'reportRound': {code = 'Math.round(<#1>)'; break;}
            case 'reportMonadic': {code = 'Math.<#1>(<#2>)'; break;}
            case 'reportRandom': {code = 'Math.round(Math.random()*(<#2>-<#1>)+<#1>)'; break;}
            case 'reportLessThan': {code = '<#1> < <#2>'; break;}
            case 'reportEquals': {code = '<#1> === <#2>'; break;}
            case 'reportGreaterThan': {code = '<#1> > <#2>'; break;}
            case 'reportAnd': {code = '(<#1> && <#2>)'; break;}
            case 'reportOr': {code = '(<#1> || <#2>)'; break;}
            case 'reportNot': {code = '!(<#1>)'; break;}
            case 'reportTrue': {code = 'true'; break;}
            case 'reportFalse': {code = 'false'; break;}
            case 'doSetVar': {code = '<#1> = <#2>;'; break;}
            case 'doChangeVar': {code = '<#1> = <#1> + (<#2>);'; break;}
            case 'doReport': {code = 'setReturnvalue(<#1>);'; break;}
            case 'reportNewList': {code = '((String("<#1>").length===0) ? new List() : new List(String("<#1>").split(" ")))'; break;}
            case 'reportCONS': {code = 'new List().cons(<#1>,<#2>)'; break;}
            case 'reportListItem': {code = '(<#2>.length() < 1 ? new String() : (String("<#1>")==="any") ? <#2>.at(Math.floor(Math.random() * (<#2>.length()) + 1)) : \n\
                                                                               (String("<#1>")==="last") ? <#2>.at(<#2>.length()) :\n\
                                                                               <#1> < 1 ? new String() :  <#2>.at(<#1>))'; break;}
            case 'reportCDR': {code = '<#1>.cdr()'; break;}
            case 'reportListLength': {code = '((<#1>.length()===0) ? 0 : <#1>.length())'; break;}
            case 'reportListContainsItem': {code = '<#1>.contains("<#2>")'; break;}
            case 'doAddToList': {code = '<#2>.add(<#1>);'; break;}
            case 'doDeleteFromList': {code = 'if(<#2>.length() > 0)\n   if("<#1>"==="all") <#2> = new List(); \n   else if("<#1>"==="last") <#2>.remove(<#2>.length()); else <#2>.remove(<#1>);'; break;}
            case 'doInsertInList': {code = 'if(<#3>.length() < 1) {<#3> = new List(); <#3>.add(<#1>);}\n   else if("<#2>"==="any") <#3>.add(<#1>,Math.floor(Math.random() * <#3>.length()) + 1);\n      else if("<#2>"==="last") <#3>.add(<#1>);\n         else <#3>.add(<#1>,<#2>);'; break;}
            case 'doReplaceInList': {code = 'if(<#2>.length() > 0) {if("<#1>"==="any") <#2>.put(<#3>,Math.floor(Math.random() * <#2>.length()) + 1);\n      else if("<#1>"==="last") <#2>.put(<#3>,<#2>.length());\n         else <#2>.put(<#3>,<#1>);}'; break;}
                
         };
        StageMorph.prototype.codeMappings[selector] = code;     
//------------------------------------------------------------------------------------------
        return newBlock;
    }
...
//--------------------------------------------------------- eingefuegt 9-12-13 --------------------------------------
        blocks.push(block('reportGetRGB'));
        blocks.push(block('doSetRGB'));
        blocks.push(block('reportRGBtoHSV')); 
//------------------------------------------------------------------------------------------------------------------
        blocks.push('-');
    ...
    // for debugging: ///////////////
        ...
        blocks.push('-');
        blocks.push(block('doWriteTextfile'));
        blocks.push(block('reportReadTextfile'));
        blocks.push(block('doDeleteTextfile'));
...
//--------------------------------------------------------- eingefügt 10-12-13 --------------------------------------
        //blocks.push(block('doMakeDynamicVariable'));
        //blocks.push(block('doDeleteDynamicVariable'));
        //blocks.push(block('doSetDynamicVariable'));
        //blocks.push(block('reportGetDynamicVariable'));
        blocks.push(block('doChangeStagesize'));
//------------------------------------------------------------------------------------------------------------------
        ...
        //--------------------------------------------------------- eingefügt 14-01-01 --------------------------------------
        blocks.push('-');
        blocks.push(block('doConnectToServer'));
        blocks.push(block('reportGetDatabases'));
        blocks.push(block('doChooseDatabase'));
        blocks.push(block('reportGetTables'));
        blocks.push(block('reportGetColumns'));
        blocks.push('-');
        blocks.push(block('doCreateDBvars'));
        blocks.push(block('doDeleteDBvars'));
        blocks.push('-');
        blocks.push(block('reportSQLequals'));
        blocks.push(block('reportSQLgreater'));
        blocks.push(block('reportSQLless'));
        blocks.push(block('reportSQLnot'));
        blocks.push(block('reportSQLand'));
        blocks.push(block('reportSQLor'));
        blocks.push(block('reportSQLin'));
        blocks.push(block('reportSQLavg'));
        blocks.push(block('reportSQLcount'));
        blocks.push(block('reportSQLmax'));
        blocks.push(block('reportSQLmin'));
        blocks.push(block('reportSQLsum'));
        blocks.push(block('reportExecSQLcommand'));
        blocks.push(block('reportSimpleSelect'));
        blocks.push(block('reportFullSelect'));
        //--------------------------------------------------------- eingefügt 14-01-01 --------------------------------------
    ...
    /////////////////////////////////
        ...
        if (StageMorph.prototype.enableCodeMapping) {
            ...
            blocks.push(block('reportExecJScode'));
            blocks.push('=');
        }
...
/*--------------------------------------------------------- eingefügt 14-01-01 --------------------------------------
  functions for access to dynamic variables in sql.js
//------------------------------------------------------------------------------------------------------------------
*/
...
//-------------------------------------------- geändert 9-12-13 ----------------------------------------------
StageMorph.prototype.dimensions = new Point(600, 400); // unscaled extent
//------------------------------------------------------------------------------------------------------------
